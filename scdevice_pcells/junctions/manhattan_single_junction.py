...
class SqnlManhattanSingleJunction(Junction):
    """SCDevice copy of the Manhattan single-junction PCell for local customization."""
    ...
    include_base_metal_addition = Param(pdt.TypeBoolean, "Include base metal addition layer.", False)
    ...
    pad_height = Param(pdt.TypeDouble, "Height of the junction pad.", 3.0, unit="μm")
    pad_width = Param(pdt.TypeDouble, "Width of the junction pad.", 3.0, unit="μm")
    ...
    def _add_refpoints(self):
        """Add the ``origin_squid`` refpoint and ``common`` port."""
    ...
    def _produce_ground_metal_shapes(self):
        """Produce hardcoded shapes in metal gap and metal addition layers."""
        x0 = -self.a / 2
        y0 = self.height / 2
        bottom_pts = [
