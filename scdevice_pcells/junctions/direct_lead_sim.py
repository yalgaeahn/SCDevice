"""Simulation junction refpoints for direct DoublePadsSQNL taper attachment."""
...
JUNCTION_TERMINAL_MODEL = "taper_edge_port"
SURROGATE_PADS_ENABLED = False
SURROGATE_PAD_WIDTH_UM = 8.0
SURROGATE_PAD_LENGTH_UM = 2.0


class SqnlDirectLeadSim(Junction):
    """Simulation-only junction that connects a lumped port directly to taper edges."""
    ...
    use_surrogate_pads = Param(
        pdt.TypeBoolean,
        "Generate legacy simulation-only metal surrogate pads.",
        False,
    )
    include_background_gap = Param(
        pdt.TypeBoolean,
        "Add base metal gap around surrogate pads when generated.",
        True,
    )
    ...
    def build(self):
        ...
        if self.use_surrogate_pads:
            ...
            lower_port = pya.DPoint(0, self.surrogate_pad_length)
            upper_port = pya.DPoint(0, self.attach_span - self.surrogate_pad_length)
        ...
        self.refpoints["port_squid_a"] = upper_port
        self.refpoints["port_squid_b"] = lower_port
