...
from scdevice_pcells.junctions.manhattan_single_junction import SqnlManhattanSingleJunction


@add_parameters_from(Squid, junction_type=SQNL_MANHATTAN_SINGLE_JUNCTION)
@add_parameters_from(SqnlManhattanSingleJunction)
...
class DoublePadsSQNL(Qubit):
    ...
    ground_gap = Param(pdt.TypeList, "Width, height of the ground gap (µm, µm)", [700, 500])
    ...
    island1_taper_width = Param(pdt.TypeDouble, "First qubit island tapering width on the island side", 30, unit="µm")
    island1_taper_junction_width = Param(
        pdt.TypeDouble, "First qubit island tapering width on the junction side", 10, unit="µm"
    )
    island2_taper_width = Param(pdt.TypeDouble, "Second qubit island tapering width on the island side", 30, unit="µm")
    island2_taper_junction_width = Param(
        pdt.TypeDouble, "Second qubit island tapering width on the junction side", 10, unit="µm"
    )

    island_island_gap = Param(pdt.TypeDouble, "Island to island gap distance", 100, unit="µm")
