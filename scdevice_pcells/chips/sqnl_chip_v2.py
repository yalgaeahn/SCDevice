...
class SqnlSingleV2(Chip):
    """The PCell declaration for a single-qubit SQNL chip v2.

    The SQNL single chip v2 has one qubit coupled by one readout resonator to a
    horizontal feedline. Compared with v1, the readout resonator starts at the
    qubit-side coupler, routes toward the feedline, follows the feedline for the
    coupling section, then meanders to a galvanically shorted endpoint.
    ...
    def _produce_readout_resonator(self, total_length, coupling_length, pos_cplr, above_feedline, resonator_index=None):
        ...
        The v2 resonator starts at the qubit-side coupler, routes toward the
        feedline, follows the feedline for the coupling section, and then
        meanders to the galvanically shorted endpoint.
        ...
        Args:
            ...
            pos_cplr: A DPoint defining the qubit coupler port, which is the open end of the resonator.
